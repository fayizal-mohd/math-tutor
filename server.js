...
app.get('/api/config/gemini', (req, res) => {
    db.get(`SELECT value FROM configuration WHERE key = ?`, ['geminiApiKey'], (err, row) => {
        ...
        const isConfigured = !!(row && row.value);
        res.json({ success: true, configured: isConfigured });
    });
});

app.delete('/api/config/gemini', (req, res) => {
    // In a real app, you'd protect this endpoint to ensure only parents can delete.
    const sql = `UPDATE configuration SET value = ? WHERE key = ?`;
    db.run(sql, ['', 'geminiApiKey'], function(err) {
        if (err) {
            console.error("Error deleting API key:", err);
            return res.status(500).json({ success: false, message: 'Error deleting API key.' });
        }
